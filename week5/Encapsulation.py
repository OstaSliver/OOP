...
def enroll_to_subject(student, subject):
    if isinstance(student, Student) and isinstance(subject, Subject) :
        ...
        for enrollment in enrollment_list:
            if enrollment.student.student_id == student.student_id and enrollment.subject.subject_id == subject.subject_id :
                return "Already Enrolled"
        enrollment_list.append(Enrolled(student, subject))
        return "Done"
    else:
        return "Error"
...
def search_subject_that_student_enrolled(student):
    if isinstance(student, Student):
        ...
        return "Error"
# TODO 8 : function สำหรับใส่เกรดลงในการลงทะเบียน โดยรับ instance ของ student และ subject (reuse function ข้อ 3)
def assign_grade(student, subject, grade):
    if isinstance(student, Student) and isinstance(subject, Subject) and isinstance(grade, str):
        if student not in student_list or subject not in subject_list:
            return "Error"
...
# TODO 9 : function สำหรับคืน instance ของอาจารย์ที่สอนในวิชา(resuse function ข้อ 1)
def get_teacher_teach(subject_search):
    if isinstance(subject_search, Subject):
        return search_subject_by_id(subject_search.subject_id).teacher    
    else:
        return "Error"
    # return [subject.teacher for subject in subject_list if subject.subject_id == subject_search.subject_id][0]


# TODO 10 : function สำหรับค้นหาจำนวนของนักศึกษาที่ลงทะเบียนในรายวิชา โดยรับ instance ของ subject (reuse function ข้อ 6)
def get_no_of_student_enrolled(subject):
    if isinstance(subject, Subject):
        return len(search_student_enroll_in_subject(subject))
    else:
        return "Error"
    # return sum(1 for enrollment in enrollment_list if enrollment.subject == subject)

# TODO 11 : function สำหรับค้นหาข้อมูลการลงทะเบียนและผลการเรียนโดยรับ instance ของ student (reuse function ข้อ 7)
# TODO : และ คืนค่าเป็น dictionary { ‘subject_id’ : [‘subject_name’, ‘grade’ }

def get_student_record(student):
    # return {enrollment.subject.subject_id : [enrollment.subject.subject_name, enrollment.grade] for enrollment in enrollment_list if enrollment.student == student}
    return {enrollment.subject.subject_id : [enrollment.subject.subject_name, enrollment.grade] for enrollment in search_subject_that_student_enrolled(student)}
...
def get_student_GPS(student):
    ...
    for enrollment in enrollment_list:
        if enrollment.student == student:
            total_credit += enrollment.subject.credit
            total_grade += enrollment.subject.credit * grade_to_count(enrollment.grade)
